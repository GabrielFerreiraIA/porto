...
SalesforceInteractions.init({
    ...
    const createLineItemFromId = (id) => {
        ...
        // A lista de produtos - adicione aqui os produtos para capturar view item. (VIEW ITEM GENERICO) - implementado em 10-03-2026
        // mapeamento de view de categoria continuar apartado
        const productMappings = [
            { path: '/seguro-auto', id: 'seguro_auto' },
            { path: '/seguro-moto', id: 'seguros_moto' },
            { path: '/seguro-bike', id: 'seguros_bike' },
            { path: '/seguro-residencial', id: 'seguro_residencial' },
            { path: '/seguro-viagem', id: 'seguro_viagem'},
            { path: '/smart-e-games/smart-e-games-hub', id: 'smart-e-games' },
            { path: '/notebook-e-tablet/notebook-e-tablet-hub', id: 'notebook-e-tablet' },
            { path: '/foto-e-video/foto-e-video-hub', id: 'foto-e-video' },
            { path: '/celular', id: 'seguros_celular_hub' },
            { path: '/seguro-auto', id: 'seguro_auto' },
            { path: '/seguro-moto', id: 'seguros_moto' },
            { path: '/seguro-bike', id: 'seguros_bike' },
            { path: '/seguro-viagem', id: 'seguro_viagem' },
            { path: '/seguro-imobiliaria', id: 'seguros_imobiliaria' },
            { path: '/protecao-combinada', id: 'seguros_protecao-combinada' },
            { path: '/seguro-responsabilidade-civil/profissional', id: 'seguros_responsabilidade-civil' },
            { path: '/seguro-fianca', id: 'seguros_fianca' },
            { path: '/maquinas-e-equipamentos', id: 'seguros_maquinas-e-equipamentos' },
            { path: '/eventos', id: 'seguros_eventos' },
            { path: '/previdencia-privada', id: 'bank_previdencia-individual' },
            { path: '/cartao-de-credito', id: 'bank_cartao-de-credito_hub' },
            { path: '/consorcio', id: 'bank_consorcio' },
            { path: '/titulo-de-capitalizacao-para-aluguel', id: 'bank_titulo-de-capitalizacao' },
            { path: '/plano-saude-pets', id: 'servicos_petlove' },
            { path: '/financiamento', id: 'bank_financiamento-de-veiculo' },
            { path: '/credito-com-garantia-imobiliaria', id: 'bank_emprestimo-garantia-imovel_hub' },
            { path: '/auto-credito', id: 'bank_emprestimo-garantia-veiculo' },
            { path: '/porto-seguro-saude', id: 'saude_seguro-saude' },
            { path: '/saude-ocupacional', id: 'saude_ocupacional' },
            { path: '/seguro-odontologico', id: 'saude_odontologico' },
            { path: '/centro-automotivo', id: 'servicos_centro-automotivo' },
            { path: '/tag-porto', id: 'bank_tag' }

        ];
        // Função auxiliar para encontrar o mapeamento de produto correspondente ao path atual (VIEW ITEM GENERICO)
        const getProductByPath = () => {
            // Itera sobre os mapeamentos para encontrar a correspondência exata de path
            const currentPath = window.location.pathname;
            const productMatch = productMappings.find(mapping => mapping.path === currentPath);
            return productMatch;
        }
            ...
            let timer = setInterval(() => {
                ...
                if (cartData && window.location.pathname === '/loja/carrinho/checkout/sucesso') {
                    ...
                    cartData.forEach((product) => {
                        ...
                        } else if (product.item_name == "seguro-auto" || product.item_name == "seguro-autoauto") {
                            SalesforceInteractions.sendEvent({
                                interaction: {
                                    name: SalesforceInteractions.OrderInteractionName.Purchase,
                                    order: {
    ...
    // Função genérica para adicionar dados de pescod e AddToCart dentro do sessionStorage
function persistDataLayer() {
    const safeReplacer = (key, value) => {
        if (value instanceof Node || typeof value === 'function' || (key && key.startsWith('__react'))) {
            return undefined;
        }
        return value;
    };
    ...
    const syncToMPC = () => {
        ...
        // Recupera o que já temos salvo para não perder dados entre as trocas de página (SPA)
        let currentStorage = JSON.parse(sessionStorage.getItem("MPCDataLayer")) || { data: [] };
        // Extrai os objetos puros do storage para facilitar a busca
        let storedEvents = currentStorage.data.map(item => item[0]);
        // Busca os alvos no Data Layer atual (pescod e add_to_cart)
        const pescodInDL = window.dataLayer.find(obj => obj && obj.porto_pescod);
        const addToCartInDL = window.dataLayer.find(obj => obj && obj.event === 'add_to_cart');
        let updated = false;
        // Se achou pescod e ele ainda não estava no storage, adiciona
        if (pescodInDL && !storedEvents.some(e => e.porto_pescod)) {
            storedEvents.push(JSON.parse(JSON.stringify(pescodInDL, safeReplacer)));
            updated = true;
        }
        // Se achou add_to_cart e ele ainda não estava no storage, adiciona
        if (addToCartInDL && !storedEvents.some(e => e.event === 'add_to_cart')) {
            storedEvents.push(JSON.parse(JSON.stringify(addToCartInDL, safeReplacer)));
            updated = true;
        }
        // Só grava no sessionStorage se houver dados novos
        if (updated) {
            currentStorage.data = storedEvents.map(obj => [obj]);
            currentStorage.updatedAt = new Date().getTime();
            sessionStorage.setItem("MPCDataLayer", JSON.stringify(currentStorage));
        }
    };
    syncToMPC();
    ...
    window.addEventListener('beforeunload', syncToMPC);
};
//fim do preenchimento do sessionStorage



//produtos de serviços mapeados
const SERVICOS_PERMITIDOS = [
    "limpeza-de-sofa", 
    "limpeza-de-ar-condicionado", 
    "instalacao-de-fechadura-digital", 
    "manutencao-preventiva-de-aquecedor-a-gas", 
    "conserto-de-eletrodomesticos",
    "instalacao-de-tv",
    "encanador",
    "desentupimento",
    "combo-limpeza-sala-de-estar",
    "eletricista",
    "instalacao-de-fogao-cooktop-ou-forno",
    "conserto-de-ar-condicionado",
    "limpeza-de-caixa-dagua",
    "impermeabilizacao-de-sofa",
    "instalacao-de-lava-loucas",
    "instalacao-de-maquina-de-lavar-ou-secadora",
    "instalacao-de-coifa-ou-depurador",
    "instalacao-de-ventilador",
    "instalacao-de-ar-condicionado-de-7-a-12-mil-btus",
    "instalacao-de-ar-condicionado-de-15-a-30-mil-btus",
    "confeccao-de-chave",
    "manutencao-de-box-de-vidro",
    "instalacao-de-filtro-ou-purificador-de-agua",
    "fixacao-de-ate-3-itens"
];
        ...
        const sitemapInstitucional = {
            ...
            pageTypes: [
                ...
                //  SEGURO DE VIDA - CARRINHO ABANDONADO - ADD TO CART
                {
                    ...
                    isMatch: () => {
                        if (window.location.pathname === '/loja/carrinho') {
                            ...
                            //código voltado para serviços e equipamento portateis, gerador de addToCart baseado no session storage
                            const mpcDataRaw = sessionStorage.getItem("MPCDataLayer");
                                        if (!mpcDataRaw) return;

                                        try {
                                            const mpcData = JSON.parse(mpcDataRaw);
                                            const events = mpcData.data ? mpcData.data.map(item => item[0]) : [];
                                            
                                            let pescod = events.find(o => o && o.porto_pescod)?.porto_pescod;
                                            let addToCartObj = events.find(o => o && o.event === "add_to_cart");
                                            const itemData = addToCartObj?.ecommerce?.items?.[0] || addToCartObj?.items?.[0];
                                             const emailSessionStorage = sessionStorage.getItem('user_email');
                                            const telefoneSessionStorage = sessionStorage.getItem('user_phone');
                                            ...
                                            if (itemData && pescod) {
                                                ...
                                                if (itemName === "servicos-para-casa-e-auto") {
                                                    const category = itemData.item_category?.trim();
                                                    if (SERVICOS_PERMITIDOS.includes(category)) {
                                                        productName = category;
                                                    }
                                                } 
                                                // VALIDAÇÃO DE EQUIPAMENTO PORTÁTEIS
                                                //else if (itemName === "equipamentos-portateis") {
                                                //    productName = itemData.item_variant?.split(':')[0].trim() || itemData.item_variant?.trim();
                                                //}
                                                ...
                                                if (productName) {
                                                    SalesforceInteractions.sendEvent({
                                                        interaction: {
                                                            name: SalesforceInteractions.CartInteractionName.AddToCart,
                                                            lineItem: createLineItemFromId(productName)
                                                        },
                                                        user: { 
                                                            identities: {
                                                                        ID_PESSOA: pescod 
                                                            ...
                                                            attributes: {
                                                                            mobilePhone: telefoneSessionStorage,
                                                                            emailAddress:emailSessionStorage
                                                                        }
                                                        }
                                                    });
                                                }
                                            }
                                        } catch (e) { console.error("Erro no AddToCart:", e); }
                                //código voltado para serviços e equipamentos portateis, gerador de addToCart baseado no session storage  
                ...
                },
                {
                    ...
                    isMatch: () => {
                        if (window.location.pathname === '/loja/carrinho/checkout/sucesso') {
                            ...
                            //mapeamento de purchase para serviços e equipamentos portateis
                            const mpcDataRaw = sessionStorage.getItem("MPCDataLayer");
                                if (!mpcDataRaw) return;
                                ...
                                try {
                                    ...
                                    if (itemData && pescod) {
                                        ...
                                        if (productName) {
                                            const d = new Date();
                                            const orderid = `${d.getMilliseconds()}-${d.getDate()}-${d.getMinutes()}`;
                                            ...
                                            SalesforceInteractions.sendEvent({
                                                interaction: {
                                                    ...
                                                    order: {
                                                        id: orderid,
                                                        lineItems: [createLineItemFromId(productName)]
                                                    },
                                                },
                                                user: { identities: { ID_PESSOA: pescod } }
                                            });

                                            if (window.location.pathname.includes('/sucesso')) {
                                                sessionStorage.removeItem("MPCDataLayer");
                                            }
                                        }
                                    }
                                } catch (e) { console.error("Erro no Purchase:", e); }
                        ...
                        listeners: [
                            ...
                            SalesforceInteractions.listener(
                                ...
                                () => {
                                    ...
                                    const mobilePhoneTratado = mobilePhone.replace(/[^\d]/g, '');
                                    if (emailAddress) sessionStorage.setItem('user_email', emailAddress);
                                    if (mobilePhoneTratado) sessionStorage.setItem('user_phone', 55 + mobilePhoneTratado);
                                }
                            ),
                    
                    
                                    SalesforceInteractions.DisplayUtils.pageElementLoaded("[data-gtm-name='loja:informacoes-para-cotacao']").then(() => {
                                    const leadEvent = (window.dataLayer || []).findLast(e => e.event === 'lead');
                                    const bcpCode = leadEvent ? leadEvent.porto_pescod : null; 
                                    const emailSessionStorage = sessionStorage.getItem('user_email');
                                    const telefoneSessionStorage = sessionStorage.getItem('user_phone');
                            ...
                            SalesforceInteractions.listener(
                                ...
                                "button[data-gtm-subname*='voltar']",
                                () => {
                                    setTimeout(() => SalesforceInteractions.reinit(), 1000);
                                }
                            )
                        ]
                    },

                    // Fluxo de contração SEGURO CELULAR
                    {
                    name: "fluxo_contratacao_seguroCelular",
                    ...
                    listeners: [
                        SalesforceInteractions.listener(
                            ...
                            () => {
                                        ...
                                        const deviceData = {
                                        marca: SalesforceInteractions.cashDom('[data-testid="select-device.brand"] .select__single-value').text().trim(),
                                        modelo: SalesforceInteractions.cashDom('[data-testid="select-device.model"] .select__single-value').text().trim()
                                        };
                                ...
                                SalesforceInteractions.listener(
                                EVENTS.click,
                                "button[data-salesforce-mcp-tracking='contratar']",(e) => {
                                    const cobertura = e.currentTarget.getAttribute('data-testid');
                                    if (cobertura) {
                                            const product = "seguro celular";
                                            const dataAtual = new Date().toISOString();
                                            // Recupera dados do dispositivo com fallback seguro
                                            const storedData = JSON.parse(sessionStorage.getItem('celularPlanData') || '{}');
                                            const { marca = 'N/A', modelo = 'N/A' } = storedData;
                                            // Busca o bcpCode
                                            const leadEvent = (window.dataLayer || []).findLast(e => e.event === 'lead');
                                            const bcpCode = leadEvent ? leadEvent.client_bcp : null;
                                            SalesforceInteractions.sendEvent({
                                                interaction: {
                                                    name: "event_click_porto-seguro-celular-hub_planos",
                                                },
                                                user: {
                                                    identities: { ID_PESSOA: bcpCode },
                                                    profileObjects: {
                                                        CarrinhoAbandonado: [{
                                                            id: "seguro-celular",
                                                            attributes: {
                        ...
                        //AddToCart
                            SalesforceInteractions.DisplayUtils.pageElementLoaded("[data-salesforce-mcp-tracking='contratar']").then(() => {
                                    const leadEvent = (window.dataLayer || []).findLast(e => e.event === 'lead');
                                    const bcpCode = leadEvent ? leadEvent.client_bcp : null;
                                    const emailSessionStorage = sessionStorage.getItem('user_email');
                                    const telefoneSessionStorage = sessionStorage.getItem('user_phone');
                ...
                // Fluxo de contração SEGURO AUTO
                {
                    name: "fluxo_contratacao_seguroAuto",
                    isMatch: () => {
                        const regex = /\/loja\/seguro-auto\/(placa-do-veiculo|email|celular|cpf|planos)/;
                    ...
                    listeners: [
                        SalesforceInteractions.listener(
                            "click",
                            "button[data-gtm-name='qual-a-placa-do-veiculo-que-voce-quer-proteger']", 
                            () => {
                                const contentVehicle = (window.dataLayer || []).findLast(e => e.event === 'select_content');
                                const marcaModeloRaw = contentVehicle ? contentVehicle.vehicle : null;
                                let marca = null;
                                let modelo = null;
                                if (marcaModeloRaw) {
                                    const partes = marcaModeloRaw.split(':');
                                    marca = partes[0];  
                                    modelo = partes[1]; 
                                if (marca) sessionStorage.setItem('marca_veiculo', marca);
                                if (modelo) sessionStorage.setItem('modelo_veiculo', modelo);
                                }
                                const placaVeiculo = SalesforceInteractions.cashDom("[data-gtm-subname='placa-do-veiculo'] input").val();
                                if (placaVeiculo) sessionStorage.setItem('placa_veiculo', placaVeiculo); 
                            }
                        ),
                        SalesforceInteractions.listener(
                            "click",
                            "button[data-gtm-name='qual-o-e-mail-de-quem-vai-contratar-o-seguro']", 
                            () => {
                                const email = SalesforceInteractions.cashDom("[data-gtm-subname='e-mail'] input").val();
                        ...
                        SalesforceInteractions.listener(
                            "click",
                            "button[data-gtm-name='qual-o-celular-da-pessoa-que-vai-contratar-o-seguro']", 
                            () => {
                                const celular = SalesforceInteractions.cashDom("[data-gtm-subname='celular'] input").val();
                                const celularFinal = "55" + (celular || "").replace(/\D/g, ""); 
                                if (celularFinal) sessionStorage.setItem('user_phone', celularFinal);
                                }
                            
                        ),
                        //AddToCart
                        SalesforceInteractions.DisplayUtils.pageElementLoaded("[data-gtm-name='escolha-o-plano-que-mais-combina-com-voce']").then(() => {
                                let pescod = findInDataLayer('porto_pescod');
                                const emailSessionStorage = sessionStorage.getItem('user_email');
                                const telefoneSessionStorage = sessionStorage.getItem('user_phone');
                                const placaSessionStorage = sessionStorage.getItem('placa_veiculo');
                                const marcaSessionStorage = sessionStorage.getItem('marca_veiculo');
                                const modeloSessionStorage = sessionStorage.getItem('modelo_veiculo');
                                    ...
                                    SalesforceInteractions.sendEvent({
                                        ...
                                        user: {
                                            ...
                                            attributes: {
                                                emailAddress: emailSessionStorage,
                                                mobilePhone: telefoneSessionStorage,
                                                marcaVeiculo: placaSessionStorage,
                                                modeloVeiculo: marcaSessionStorage,
                                                placaVeiculo: modeloSessionStorage
               ...
               // Page Type Otimizado APENAS para Páginas de Produto, lista de produtos no topo do sitemap (VIEW ITEM GENERICO)
                {
                    name: "Página de Produto ",
                    isMatch: () => !!getProductByPath(),
                    interaction: {
                ...
                // View catalog Emprestimo e Financiamento
                {
                ...
                {
                    name: "fluxo_contratacao_servicos",
                    isMatch: () => window.location.pathname.includes("/loja/servicos/dados-cadastrais"),
                    onAction: () => {
                        // Ativa a escuta do Data Layer assim que a página carrega
                        persistDataLayer();
                    },
                    listeners: [
                        SalesforceInteractions.listener(
                            ...
                            () => {
                                persistDataLayer();
                                setTimeout(persistDataLayer, 100);
                                const emailAddress = SalesforceInteractions.cashDom("input[data-gtm-subname='email']").val();
                                const mobilePhone = SalesforceInteractions.cashDom("input[ta-gtm-subname='celular-com-ddd']").val();
                                const mobilePhoneTratado = mobilePhone.replace(/[^\d]/g, '');
                                if (emailAddress) sessionStorage.setItem('user_email', emailAddress);
                                if (mobilePhoneTratado) sessionStorage.setItem('user_phone', 55 + mobilePhoneTratado);
                            }
    ...
    //mapeamento para carrinho abandonado de equipamentos portateis
                            {
                name: 'fluxo_contratacao_equipamentosPortateis',
                    isMatch: () => {
                    const regex = /\/loja\/seguro-de-equipamentos\/(smart-e-games|foto-e-video|notebook-e-tablet)\/(dados-do-equipamento|shortform|planos|dados-complementares)/;
                    return regex.test(window.location.pathname);
                },

                onAction: () => {
                    // Ativa a escuta em todas as páginas do fluxo
                    persistDataLayer();
                    console.log("onAction_mcp");
                },
                listeners: [
                    // Focamos o clique apenas na última etapa (dados-complementares)
                    SalesforceInteractions.listener(
                        "click",
                        "[data-gtm-name='para-continuar-complete-os-dados-abaixo']",
                        () => {
                            // Força a última verificação antes do redirect para o carrinho
                            console.log("onAction_mcp");
                            persistDataLayer();
                            setTimeout(persistDataLayer, 100); 
                        }
                    ),
                    SalesforceInteractions.listener("click","button[data-gtm-name='agora-queremos-conhecer-um-pouco-mais-sobre-voce']", () => {     
                                const emailAddress = SalesforceInteractions.cashDom("input[data-gtm-subname='email']").val();
                                const mobilePhone = SalesforceInteractions.cashDom("input[data-gtm-subname='telefone']").val(); 
                                const mobilePhoneTratado = mobilePhone.replace(/[^\d]/g, '');
                                if (emailAddress) sessionStorage.setItem('user_email', emailAddress);
                                if (mobilePhoneTratado) sessionStorage.setItem('user_phone', 55 + mobilePhoneTratado);   
                    ...
                    SalesforceInteractions.DisplayUtils.pageElementLoaded(".carousel__track").then(() => {
                                                let pescod = findInDataLayer('porto_pescod');
                                                const emailSessionStorage = sessionStorage.getItem('user_email');
                                                const telefoneSessionStorage = sessionStorage.getItem('user_phone');
                                                const pathParts = window.location.pathname.split('/');
                                                const productName = pathParts[3] || 'equipamento-portatil';
                                                SalesforceInteractions.sendEvent({
                                                    interaction: {
                                                                 name: SalesforceInteractions.CartInteractionName.AddToCart,
                                                                lineItem: createLineItemFromId(productName)
...
//Carrinho abandonado de CGI (crédito com garantia de imóvel)
{
    name: "fluxo_contratacao_CGI",
    isMatch: () => {
        const regex = /\/credito-com-garantia-imobiliaria\/simulacao\/(boas-vindas|dados-pessoais|resumo)/;
        return regex.test(window.location.pathname);
    },
    ...
    listeners: [
        SalesforceInteractions.listener(
            "click",
            "button[data-gtm-name='agora-so-faltam-algumas-informacoes-suas']", 
            () => {
                const email = SalesforceInteractions.cashDom("[data-gmt-subname='email']").val();
                const telefoneRaw = SalesforceInteractions.cashDom("[data-gmt-subname='telefone']").val();
                const telefoneFinal = "55" + (telefoneRaw || "").replace(/\D/g, "");
                let pescod = findInDataLayer('porto_pescod');

                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: SalesforceInteractions.CartInteractionName.AddToCart,
                            lineItem: createLineItemFromId("bank_emprestimo-garantia-imovel_hub")
                        },
                        user: {
                            identities: {
                                ID_PESSOA: pescod,
                                emailAddress: email,
                                mobilePhone: telefoneFinal
                            }
                        }
                    });
            }
         ...
         SalesforceInteractions.listener(
            "click",
            "button[data-gtm-name='resumo-da-proposta']", 
            () => {
                const d = new Date();
                let orderid = d.getMilliseconds() + " " + d.getDay() + " " + d.getMinutes();
                let pescod = findInDataLayer('porto_pescod');
                    SalesforceInteractions.sendEvent({
                        interaction: {
                            ...
                            order: {
                            id: orderid,
                            lineItems: [createLineItemFromId("bank_emprestimo-garantia-imovel_hub")]
                            },
                        },
                        user: {
                            identities: {
                    ...
                    SalesforceInteractions.sendEvent({
								interaction: {
									name: "event_purchase_bank_emprestimo-garantia-imovel_hub_sucesso",
								},
							});
                }
